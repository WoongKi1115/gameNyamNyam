...
# word2vec_model.wv.intersect_word2vec_format('GoogleNews-vectors-negative300.bin', lockf=1.0, binary=True) # pre-trained data
...
def get_document_vectors(document_list, model):
    ...
    for row in document_list:
        ...
        if doc2vec is not None:
            ...
            mytest_index.append(line.index)
        else:
            print("있어")
            print(line)
...
document_embedding_list = get_document_vectors(game_df, word2vec_model)
...
for appid in owned_games:
    ...
    if(game_df[game_df['appid']==appid].empty):
        ...
        appid_list.append(temp['name'].values[0])
...
for game_name in appid_list:
    print(game_name)
    try:
        recommend_info = info_recommendations(game_name, game_df, cosine_similarities)
...
random_games = random_games.sample(n=60)
